...
namespace my_loader {
  ...
  static int
  load_dynsym_bfd(bfd *bfd_h, my_loader::Binary *bin)
  {
    int ret;
    long n, nsyms, i;
    asymbol **bfd_dynsym;
    ...
    if(n < 0) {
      ...
      for(i = 0; i < nsyms; i++) {
        if(bfd_dynsym[i]->flags & BSF_FUNCTION) {
          bin->symbols.push_back(my_loader::Symbol());
          sym = &bin->symbols.back();
          sym->type = my_loader::Symbol::SYM_TYPE_FUNC;
  ...
  static int
  load_sections_bfd(bfd *bfd_h, my_loader::Binary *bin)
  {
    int bfd_flags;
    uint64_t vma, size;
    const char *secname;
    ...
    my_loader::Section::SectionType sectype;

    for(bfd_sec = bfd_h->sections; bfd_sec; bfd_sec = bfd_sec->next) {
      bfd_flags = bfd_section_flags(bfd_sec);

      sectype = my_loader::Section::SEC_TYPE_NONE;
      if(bfd_flags & SEC_CODE) {
        sectype = my_loader::Section::SEC_TYPE_CODE;
      } else if(bfd_flags & SEC_DATA) {
        sectype = my_loader::Section::SEC_TYPE_DATA;
      } else {
        continue;
      }
      ...
      bin->sections.push_back(my_loader::Section());
  ...
  static int
  load_binary_bfd(std::string &fname, my_loader::Binary *bin, my_loader::Binary::BinaryType type)
  {
    int ret;
    bfd *bfd_h;
    ...
    bfd_h = my_loader::open_bfd(fname);
    if(!bfd_h) {
      goto fail;
    }
    ...
    case bfd_target_elf_flavour:
      bin->type = my_loader::Binary::BIN_TYPE_ELF;
      break;
    case bfd_target_coff_flavour:
      bin->type = my_loader::Binary::BIN_TYPE_PE;
      break;
    ...
    case bfd_mach_i386_i386:
      bin->arch = my_loader::Binary::ARCH_X86; 
      bin->bits = 32;
      break;
  ...
  int
  load_binary(std::string &fname, my_loader::Binary *bin, my_loader::Binary::BinaryType type)
  {
  ...
  void
  unload_binary(my_loader::Binary *bin)
  {
    size_t i;
    my_loader::Section *sec;
